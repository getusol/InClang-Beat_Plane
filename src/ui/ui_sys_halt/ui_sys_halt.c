...
/**
 * @brief sys_halt的ui界面 第一阶段 初始化 根屏幕或者其它共享资源
 */
void ui_sys_halt_init_stage1()
{
 ...
 * @brief sys_halt的ui界面
 */
void ui_sys_halt_init_stage2()
{
    // Parent
    if (sys_halt_display == NULL)
    {
        CONSOLE_ERROR("sys_halt_display is NULL.cannot create sys_halt interface.");
        LOG_ERROR("sys_halt_display is NULL.cannot create sys_halt interface.");
        return;
    }
