...
static lv_obj_t *plane_objs[CHARACTER_ID_MAX] = {NULL};
static lv_obj_t *lock_masks[CHARACTER_ID_MAX] = {NULL};

static lv_obj_t *choosed_indicator = NULL;
static lv_obj_t *detail_panel = NULL;
static lv_obj_t *choose_btn = NULL;
static lv_obj_t *choose_lbl = NULL;
static lv_obj_t *hp_label = NULL;
static lv_obj_t *dmg_label = NULL;
static lv_obj_t *skill_label = NULL;

static lv_obj_t *base_exit_popup = NULL;
static int current_viewing_idx = 0;

// 飞机固有属性配置表
static const plane_info_t plane_templates[CHARACTER_ID_MAX] = {
...
static char plane_path_buf[CHARACTER_ID_MAX][64];

#ifdef SIMULATOR
static lv_img_dsc_t base_bg_dsc;
static lv_img_dsc_t plane_base_dsc;
static lv_img_dsc_t plane_dscs[CHARACTER_ID_MAX];
...
void ui_base_run(void)
{
    ...
    current_viewing_idx = g_selected_character_id;
    update_detail_panel(current_viewing_idx);

    /* SELECTED 标签 */
    lv_obj_align_to(choosed_indicator, plane_objs[g_selected_character_id],
...
static void choose_btn_cb(lv_event_t *e)
{
    ...
    if (!g_character_unlocked[current_viewing_idx])
    {
        CONSOLE_INFO("Cannot choose: Plane %s is locked.", plane_templates[current_viewing_idx].name);
        return;
    }

    g_selected_character_id = (character_id_t)current_viewing_idx;
    lv_obj_align_to(choosed_indicator, plane_objs[g_selected_character_id], LV_ALIGN_OUT_TOP_MID, 0, -8);

    CONSOLE_INFO("Successfully switched to aircraft: %s", plane_templates[g_selected_character_id].name);
}
