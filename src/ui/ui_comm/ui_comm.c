...
/**
 * @brief 初始化comm 根屏幕 第一阶段
 */
void ui_comm_init_stage1(void)
{
    // UI_COMM 根容器
...
/**
 * @brief 初始化comm全部组件 第二阶段
 */
void ui_comm_init_stage2(void)
{
    ...
    if (dp_comm == NULL)
    {
        CONSOLE_ERROR("dp_comm is NULL,cannot create UI_COMM interface");
        LOG_ERROR("dp_comm is NULL,cannot create UI_COMM interface");
        return;
    }
