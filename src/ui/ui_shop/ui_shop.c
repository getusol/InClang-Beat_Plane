...
#else
                img = lv_img_create(cell); lv_img_set_src(img, img_path(PLAYER_EMBER_IMG, cell_path_bufs[i], 64));
#endif
...
#else
                img = lv_img_create(cell); lv_img_set_src(img, img_path(PLAYER_STREAM_IMG, cell_path_bufs[i], 64));
#endif
...
#else
                img = lv_img_create(cell); lv_img_set_src(img, img_path(PLAYER_VERDANT_IMG, cell_path_bufs[i], 64));
#endif
...
#endif
                if (i == 1) lv_label_set_text(label, "x5");
                if (i == 3) lv_label_set_text(label, "x10");
                if (i == 5) lv_label_set_text(label, "x20");
                if (i == 7) lv_label_set_text(label, "x200");
                break;
        }
        if (img) lv_obj_align(img, LV_ALIGN_CENTER, 0, -10);
    }
...
static void draw_btn_event_cb(lv_event_t * e)
{
    if (roulette_timer != NULL) return;
    if (!lv_obj_has_flag(reward_popup, LV_OBJ_FLAG_HIDDEN)) return; 
...
#ifdef SIMULATOR
        if (reward_img != NULL) { lv_obj_del(reward_img); reward_img = NULL; }
#else
