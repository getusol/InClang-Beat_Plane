...
#define BULLET_MAX_X 1024                 // 子弹最大X坐标
#define BULLET_MIN_X 0                    // 子弹最小X坐标
#define BULLET_MAX_Y 600                  // 子弹最大Y坐标
...
void bullet_init(lv_obj_t * parent)
{
    ...
    for (uint16_t i = 0;i < MAX_BULLET_COUNT;i++)
    {
        ...
        bullets[i].base.w = 16;
        bullets[i].base.h = 16;
        bullets[i].base.hitbox_x = 5;
        bullets[i].base.hitbox_y = 2;
        bullets[i].base.hitbox_h = 12;
        ...
        bullets[i].base.obj = img_create_from_array(parent,img_path(BULLET_IMG_NAME,bullet_img_path,64),bullets[i].base.w,bullets[i].base.h,bullet_img_buf,&bullet_img_struct,true);
        lv_obj_set_align(bullets[i].base.obj,LV_ALIGN_TOP_LEFT);

        #if SHOW_HITBOX
        bullets[i].base.hitbox_obj = NULL;
        bullets[i].base.hitbox_obj = game_obj_hitbox_init(&bullets[i].base);
        #endif
