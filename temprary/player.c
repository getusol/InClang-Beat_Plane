...
void player_set_shield_active(game_obj_t *obj, bool active)
{
  if (obj == NULL)
  {
    CONSOLE_WARNING("Player object is not initialized. Cannot set shield active.");
    LOG_WARNING("Player object is not initialized. Cannot set shield active.");
    return;
  }
  if (!obj->active)
  {
  ...
  player_t *p = (player_t *)obj;
  p->shield_active = active;
  if (active)
  {
    lv_obj_clear_flag(p->shield_overlay, LV_OBJ_FLAG_HIDDEN);
  }
  else
  {
...
static void player_event_game_start_cb(game_obj_t *src, game_obj_t *trg)
{
  ...
  if (mp_get_state() == MP_STATE_CONNECTED)
  {
    ...
    player_p2->base.x = 624;
    player_p2->base.y = 500;
...
static void p2_player_init(lv_obj_t *parent)
{
  ...
  if (player_p2 == NULL)
  {
    CONSOLE_ERROR("P2 malloc failed");
    return;
  }
...
{
#ifdef SIMULATOR
  if (obj == (game_obj_t *)player_p2)
    return player_p2 && player_p2->shield_active;
#endif
...
uint32_t player_get_skill_x_elapsed(void)
{
  if (!player_p)
    return 0;
  if (player_p->skill_x_last_use == 0)
    return 0xFFFFFFFF; // 从未使用，始终就绪
...
uint32_t player_get_skill_y_elapsed(void)
{
  if (!player_p)
    return 0;
  if (player_p->skill_y_last_use == 0)
    return 0xFFFFFFFF;
...
{
  if (!player_p2 || plane_id < 0 || plane_id >= TOTAL_PLANES)
    return;
  const plane_config_t *cfg = &plane_configs[plane_id];
...
uint32_t player_get_p2_skill_x_elapsed(void)
{
  if (!player_p2)
    return 0;
  if (player_p2->skill_x_last_use == 0)
    return 0xFFFFFFFF;
...
uint32_t player_get_p2_skill_y_elapsed(void)
{
  if (!player_p2)
    return 0;
  if (player_p2->skill_y_last_use == 0)
    return 0xFFFFFFFF;
...
static void p2_player_update(game_obj_t *g)
{
  ...
  g->show(g);
  if (gs != GS_PLAY || !player_p2->base.active)
    return;
  ...
  if (player_p2->speed_boost_active)
  {
    g->vx *= 2;
    g->vy *= 2;
  }
...
static void p2_player_fire(void)
{
  if (!player_p2 || fsm_get_state() != GS_PLAY || !player_p2->base.active)
    return;
  game_obj_t *g = &player_p2->base;
...
static int16_t player_hp_modify_p2(int16_t delta)
{
  if (!player_p2)
    return 0;
  if (!player_p2->base.active)
    return player_p2->hp;
  player_p2->hp += delta;
  if (player_p2->hp > player_p2->hp_max)
    player_p2->hp = player_p2->hp_max;
  if (player_p2->hp <= 0)
  {
    player_p2->hp = 0;
